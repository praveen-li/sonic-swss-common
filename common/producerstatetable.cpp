...
ProducerStateTable::ProducerStateTable(RedisPipeline *pipeline, const string &tableName, bool buffered)
    ...
    , m_pipe(pipeline)
{
    /* We only publish when keyset was empty before.
     * This is to avoid building the notification channel messages
     * which could cause memory increasely growing in case of slow consumer
     * We assume consumer will take all keys in the keyset with one
     * notification or it should feed notifications by itself if there
     * are keys remaining
     *
     * Note: ketset_num below is the number of keys in the keyset
     * Additional caution was taken care here by sending more notification
     * purposely, every 128 keys, we send additional notification to consumer
     * This is to avoid the case where one notification is missing (shouldn't
     * happen in the first place) and the whole consumer will be starving.
     */
    string luaSet =
